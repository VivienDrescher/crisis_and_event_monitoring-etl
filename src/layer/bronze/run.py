...
from src.utils.io import download_file_from_url, load_yaml
    ...
    for table_name in table_names:
        ...
        if aquisition_method == "manual_drop":
            ...
            for date in reversed(
                get_date_range(pipeline_start_date, pipeline_end_date)
            ):
        ...
        for file_count, candidate in enumerate(file_candidates, start=1):
            ...
            if ingestion_mode != "overwrite" and bronze_path.exists():
                ...
                if ingestion_mode == "latest_snapshot":
                    ...
                    prefixed_logger.info(
                        f"[Latest snapshot] Using file: {input_file.name}"
                    )
