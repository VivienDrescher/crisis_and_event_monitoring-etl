...
def build(df: pd.DataFrame, logger: Optional[logging.Logger] = None) -> pd.DataFrame:
    ...
    if month_col not in df.columns or year_col not in df.columns:
        logger.warning(
            f"[custom_transform_acled] Columns {month_col}/{year_col} missing; skipping transformation"
        )
        return df

    try:
        # Convert month abbreviations to integer month (1-12)
        month_numbers = pd.to_datetime(
            df[month_col], format="%B", errors="coerce"
        ).dt.month
        df["month_start_date"] = pd.to_datetime(
            dict(year=df[year_col], month=month_numbers, day=1),
            errors="coerce",
            utc=True,
        )
        ...
        if num_missing > 0:
            logger.warning(
                f"[custom_transform_acled] {num_missing} rows could not be converted to 'month_start_date'"
            )
        else:
            logger.info(
                "[custom_transform_acled] 'month_start_date' successfully created"
            )
    except Exception:
