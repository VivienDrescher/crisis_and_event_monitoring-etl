...
if ingestion_mode == "latest_snapshot":
    ...
    df, processed_input_files = process_bronze_to_silver(
        [latest_file], table_name, silver_schema, run_id, timezone, prefixed_logger
    )
...
# MODE: overwrite (full rebuild)
    ...
    df, processed_input_files = process_bronze_to_silver(
        bronze_files, table_name, silver_schema, run_id, timezone, prefixed_logger
    )
...
# MODE: append (incremental)
    ...
    df_new, processed_input_files = process_bronze_to_silver(
        new_bronze_files, table_name, silver_schema, run_id, timezone, prefixed_logger
    )
    if df_new is None:
        logger.info("No Bronze data found. Exiting.")
        sys.exit(0)
